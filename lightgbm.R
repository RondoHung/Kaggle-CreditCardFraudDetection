...
setwd("C:/Users/hu/Documents/STAT4011")
...
pred <- predict(lgb, data.matrix(test)) ##save to this line

imp <- lgb.importance(lgb)
sub <- data.frame(read_csv("./sample_submission.csv"))
sub[,2] <- pred

write.csv(sub,"LightGBM01.csv",row.names = F)
write.csv(imp,"feature_importance01.csv",row.names = F)
